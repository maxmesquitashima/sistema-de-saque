...
This project implements a function to calculate bank balance based on a sequence of deposit and withdrawal operations.

## How it works

The `calcularSaldo()` function receives a string containing banking operations separated by commas, processes each operation and returns the final balance.
...
Este projeto implementa uma função para calcular o saldo bancário baseado em uma sequência de operações de depósito e saque.

## Funcionamento

A função `calcularSaldo()` recebe uma string contendo operações bancárias separadas por vírgula, processa cada operação e retorna o saldo final.
